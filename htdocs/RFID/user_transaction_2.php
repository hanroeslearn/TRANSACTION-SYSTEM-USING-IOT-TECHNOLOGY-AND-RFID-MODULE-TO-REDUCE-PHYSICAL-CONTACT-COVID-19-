<?php
	...
	$file = fopen('balance2.txt', 'w');
	$text = $data['saldo'];
	fwrite($file, $text);
	fclose($file);
	
	$trigger = fopen('trigger2.txt', 'r');
	$trigger_read = fread($trigger,filesize("trigger2.txt"));
	fclose($trigger);
	
	if($trigger_read == "1")
	{
		$file3 = fopen('trigger2.txt', 'w');
		$text3 = "0";
		fwrite($file3, $text3);
		fclose($file3);
	}
